...
const DeckCardList = ({
  ...
  <div>
    <h3 className="text-xl font-semibold text-slate-300 mb-2 border-b border-slate-700 pb-2">
      {title} ({cards.reduce((acc, c) => acc + c.quantity, 0)} Cartas)
    </h3>
    <div className="flex-grow overflow-y-auto pr-2 custom-scrollbar">
      {cards.length > 0 ? (
        cards.map((cardInDeck) => {
          ...
          return (
            <div
              key={cardInDeck.id || `${cardInDeck.card_id}-${cardInDeck.board}`}
              className="flex items-center justify-between mb-2 p-2 rounded-md hover:bg-slate-700 transition-colors"
            >
              <div className="flex items-center min-w-0">
                <div className="w-10 h-14 flex-shrink-0 mr-3 border border-slate-600 rounded-sm overflow-hidden bg-slate-700">
                  {imageUrl && (
                    <img
                      src={imageUrl}
...
export function DeckBuilderPage() {
  ...
  const updateDeckCardMutation = useMutation({
    ...
    onError: (err, variables, context) => {
      queryClient.setQueryData(deckCardsQueryKey, context.previousDeckCards);
    },
    onSuccess: () => {
  ...
  const handleAddCardToDeck = (cardInCollection, board) => {
    ...
    if (existingCard) {
      ...
      const cardToAdd = {
        card_id: cardInCollection.card_id,
        card_name: cardInCollection.card_name,
      };
  ...
  const totalSideboardCards = sideboardCards.reduce(
    (sum, card) => sum + card.quantity,
    0
  );
  ...
  if (!deck) {
    return (
      <div className="w-full min-h-screen bg-slate-900 text-white flex flex-col items-center justify-center p-4">
        ...
        <Link to="/decks">
          <Button>Voltar para Meus Decks</Button>
        </Link>
      </div>
    );
  }
    ...
    <div className="w-full min-h-screen bg-slate-900 text-white p-4 flex flex-col">
      <div className="container mx-auto flex-grow">
        <div className="flex justify-between items-center mb-6">
          <Link
            to="/decks"
            className="text-primary-400 hover:text-primary-300 inline-block"
          >
                  ...
                  >
                    <div className="flex items-center min-w-0">
                      <img
                        ...
                        className="w-10 h-14 flex-shrink-0 mr-3 border border-slate-600 rounded-sm object-contain bg-slate-700"
                      />
                    ...
                    <div className="flex gap-1">
                      <Button
                        size="sm"
                        onClick={() =>
                          handleAddCardToDeck(cardInCollection, "mainboard")
                        }
                        disabled={
                          !canAddToDeck || updateDeckCardMutation.isLoading
                        }
                        className="bg-blue-600 hover:bg-blue-700 text-xs px-2 py-1 disabled:bg-slate-600"
                      >
                        Deck
                      </Button>
                      <Button
                        size="sm"
                        onClick={() =>
                          handleAddCardToDeck(cardInCollection, "sideboard")
                        }
                        disabled={
                          !canAddToDeck ||
                          updateDeckCardMutation.isLoading ||
                          totalSideboardCards >= 15
                        }
                        className="bg-purple-600 hover:bg-purple-700 text-xs px-2 py-1 disabled:bg-slate-600"
                      >
                        Side
                      </Button>
                    </div>
